...
| --registry-ecr-mandatory                         | `false`                  | exit with an error if the AWS API (and thereby ECR) is not accessible. Useful for escalating misconfiguration or outages that might otherwise go undetected.
